...
// Narrative loader
const { loadNarrative } = require('./narrativeLoader');
...
async function isOnCooldown(wallet) {
    return (await redis.ttl(`cooldown:${wallet}`)) > 0;
}

async function setCooldown(wallet) {
    await redis.set(`cooldown:${wallet}`, '1', 'EX', COOLDOWN);
}
...
// Load narrative data (additive, does not affect core game data)
...
// Narrative API routes (read-only, additive)
app.get('/api/narrative/main', (req, res) => {
    res.json(NARRATIVE.main);
});
...
app.get('/api/narrative/dialog/:npc', (req, res) => {
    const npc = req.params.npc;
    const dlg = NARRATIVE.dialog[npc];
    if (!dlg) return res.status(404).json({ error: 'NPC dialog not found' });
    res.json(dlg);
});

app.get('/api/narrative/terminals', (req, res) => {
    res.json(NARRATIVE.terminals);
});

app.get('/api/narrative/encounters', (req, res) => {
    res.json(NARRATIVE.encounters);
});

app.get('/api/narrative/collectibles', (req, res) => {
    res.json(NARRATIVE.collectibles);
});
